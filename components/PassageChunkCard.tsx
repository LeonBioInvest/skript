...
import { alignWords, WordDiffOp } from '@/lib/tokenizer';
  ...
  return (
    <div id={`chunk-${idx}`} className={cardClass}>
      ...
      <div className="flex gap-3 mb-4">
        <div className="flex-1 flex items-start gap-2 bg-surface2 border border-border rounded-xl px-4 py-3">
        ...
        <div className="flex items-start gap-2 bg-surface2 border border-border rounded-xl px-4 py-3">
          <span className="font-mono text-[10px] tracking-wider uppercase text-text3 mt-0.5 flex-shrink-0">Ende:</span>
          <span className="text-sm text-blue italic leading-relaxed">… {lastWord}</span>
        </div>
      </div>
      ...
      {state.checked && !state.skipped && (
        <div className="mt-4 rounded-xl border border-border bg-surface2 px-4 py-3">
          ...
          <DiffView given={state.passageValue} answer={pc.rawText} />
        </div>
      )}
...
function DiffView({ given, answer }: { given: string; answer: string }) {
  const ops = alignWords(given, answer);

  return (
    <p className="text-base leading-[2.2] text-text">
      {ops.map((op, i) => {
        if (op.type === 'extra') return null;
        ...
        if (op.type === 'missing') {
          return (
            <span
              key={i}
              className="inline-block bg-err-dim border border-err/30 rounded-[6px] px-2 py-0.5 font-mono text-[0.85rem] mx-0.5 text-err/70 whitespace-nowrap"
            >
              ∅ <span className="text-accent font-medium">{op.correct}</span>
            </span>
          );
        }
        ...
        return (
          ...
          >
            <span className="text-err line-through opacity-75">{op.given}</span>
