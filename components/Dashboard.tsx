...
import { loadHistory, clearHistory as clearHistoryStorage } from '@/lib/types';
      ...
      {activeTab === 'results' && (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="flex justify-center gap-3 pt-2 flex-wrap">
            <button
              onClick={onRestart}
              className="px-6 py-2.5 rounded-xl bg-accent-dim hover:bg-accent text-white font-semibold text-sm transition-all shadow-sm shadow-accent/20"
            >
              ↺ Neu starten
            </button>
            {onRestartErrors && wrong > 0 && (
              <button
                onClick={onRestartErrors}
                className="px-6 py-2.5 rounded-xl border border-err/40 bg-err/[0.08] text-err hover:bg-err/15 hover:border-err/60 text-sm font-semibold transition-all"
              >
                Fehler üben ({wrong})
              </button>
            )}
            <button
              ...
              className="px-6 py-2.5 rounded-xl border border-border bg-surface2 text-text hover:bg-surface3 hover:border-text3/50 text-sm font-semibold transition-all"
            >
              ← Skript wechseln
            </button>
          </div>
        ...
        <div className="animate-fade-in">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              ...
              className="font-mono text-[10px] uppercase tracking-wider text-text2 hover:text-err border border-border hover:border-err/60 bg-surface2 hover:bg-err/[0.06] px-3 py-1.5 rounded-lg transition-all"
            >
              Verlauf löschen
            </button>
          </div>
